            ...
            <div class="col-sm-12">
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        Current b3 servers
                        @can('manage', \Stormyy\B3\Models\B3Server::class)<a href="{{url('/b3/add')}}"
                    ...
                    <div class="panel-body">
                        <table class="table table-striped">
                            ...
                            @foreach($servers as $server)
                                <tr>
                                    <td>{{$server->name}} <a href="cod4://{{$server->host}}:{{$server->port}}"
                                                             title="Join {{$server->name}} server"><i
                                                    class="fa fa-sign-in"></i></a></td>
                                    <td>@if($server->slots == null)
                                            <font color="red">Offline</font>
                                        @else
                                            @if($server->online === -1 && $server->slots === -1)
                                            ...
                                            @else
                                                <div class="progress">
                                                    <div class="progress-bar progress-bar-success progress-bar-striped active"
                                                         ...
                                                         style="width:{{round($server->online/$server->slots*100)}}%">
                                    ...
                                    <td style="text-align:right;">
                                        <a class="btn btn-primary" href="{{url('/b3/'.$server->id.'/players')}}">
                                            View
                                        </a>
                                        @can('manage', $server)
                                            <a class="btn btn-primary" href="{{url('/b3/'.$server->id.'/edit')}}">
                                                Edit
                                            </a>
                                        @endcan
                                    </td>
